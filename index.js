...
//#region Client 
const activationPackageDefinition = protoLoader.
loadSync(path.join(__dirname, './serverProtos/Activation.proto'));
const activationProto = grpc.loadPackageDefinition(activationPackageDefinition);

const ConfigurationPackageDefinition = protoLoader.
loadSync(path.join(__dirname, './serverProtos/GeneralConfiguration.proto'));
const ConfigurationProto = grpc.loadPackageDefinition(ConfigurationPackageDefinition);

const sabyprofilePackageDefinition = protoLoader.
loadSync(path.join(__dirname, './serverProtos/sabyProfile.proto'));
const sabyprofileProto = grpc.loadPackageDefinition(sabyprofilePackageDefinition);





const countryPackageDefinition = protoLoader.
loadSync(path.join(__dirname, './serverProtos/Countries.proto'));
const countryProto = grpc.loadPackageDefinition(countryPackageDefinition);  

const connectPackageDefinition = protoLoader.
loadSync(path.join(__dirname, './serverProtos/ConnectNode.proto'));
const connectProto = grpc.loadPackageDefinition(connectPackageDefinition);


const socketCallDefinition = protoLoader.
loadSync(path.join(__dirname, './serverProtos/SocketCall.proto'));
const socketCallProto = grpc.loadPackageDefinition(socketCallDefinition);


const DoJobDefinition = protoLoader.
loadSync(path.join(__dirname, './serverProtos/DoJob.proto'));
const DoJobProto = grpc.loadPackageDefinition(DoJobDefinition);



const countriesService = 
new countryProto.Countries('65.109.78.162:80',
grpc.credentials.createInsecure());

global.ConfigurationService = 
new ConfigurationProto.GeneralConfigurations('65.109.78.162:80',
grpc.credentials.createInsecure());

global.sabyService = 
new sabyprofileProto.SabysProfiles('65.109.78.162:80',
grpc.credentials.createInsecure());



global.ServerActivation =
new activationProto.Activations('65.109.78.162:80',
grpc.credentials.createInsecure());

global.SocketCall =
    new socketCallProto.SocketCall('65.109.78.162:80',
        grpc.credentials.createInsecure());

global.DoJob =
    new DoJobProto.DoJob('65.109.78.162:80',
        grpc.credentials.createInsecure());

//#endregion
//#region Server
const packageDefinition = protoLoader.
                            loadSync(path.join(__dirname, './protos/test.proto'));
const processingProto = grpc.loadPackageDefinition(packageDefinition);

const ActivationpackageDefinition = protoLoader.
                            loadSync(path.join(__dirname, './protos/NodeActivation.proto'));
const activationProtoservice = grpc.loadPackageDefinition(ActivationpackageDefinition);

const InitializepackageDefinition = protoLoader.
                            loadSync(path.join(__dirname, './protos/Initialize.proto'));
const initializeProtoservice = grpc.loadPackageDefinition(InitializepackageDefinition);

const testModeServicesDefinition = protoLoader.
                            loadSync(path.join(__dirname, './protos/TestModes.proto'));
const testModeProtoServices = grpc.loadPackageDefinition(testModeServicesDefinition);

const behaviorDefinition = protoLoader.
loadSync(path.join(__dirname, './protos/Behavior.proto'));
...
server.addService(activationProtoservice.NodeActivation.service,
    { StartActivation:activationService.startActivation,StartManualActivation:activationService.StartManualActivation });
...
try{
    helper.getEndPoint(true).then(async (endPoint) => {
            ...
            server.bindAsync(endPoint, grpc.ServerCredentials.createInsecure(),
                ...
                server.start();
                await new SabyInfoRepository().UpdateStatus("Preparing", null,"Working");
            });

            if (process.argv[3] != null && process.argv[3] === 'Activation') {
                console.log(`process.argv[3] ${process.argv[3]}`)
                console.log("confirm ready activation");
                await new SabyInfoRepository().UpdateStatus("Saby Ready To Activate",null,"NULL");
                return
            }
