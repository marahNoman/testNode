   ...
   async init(){
      try{
        ...
        return true;
         ...
         await new SabyInfoRepository().UpdateStatus("An Error Occurred while init MainScript");
        return false;
      }
    }
    ...
    async validate(alpha,countryId,apiRoute){
      try{
         ...
         if(!isVpnInstalled){
             await new SabyInfoRepository().UpdateStatus("ACTIVATING", null, "INSTALLING VPN");
             await avdScript.installApp("../SabyDemo/apk/wireguard.apk");
            var checkInstalled = await checker.isWireguardAppInstalled();
            if(!checkInstalled){
              await new SabyInfoRepository().UpdateStatus("KILLED", null, 'Wireguard VPN NOT INSTALLED');
              return false;
            }
         }
         await avdScript.installApp("../SabyDemo/apk/IPLocation.apk");
         console.log(`ConnectWireGuard: ${alpha},${countryId},${apiRoute}`)
         await new VpnHelper(avdScript.getEmulatorPort()).ConnectWireGuard(alpha,countryId,apiRoute);



         if(!await checker.isVpnConnected()){
                await new SabyInfoRepository().UpdateStatus("KILLED", null, "ACTIVATION VPN NOT CONNECTING");
                return false;
         }
         ...
         if(!isinternet){
          ...
          if(!CheckProxmox){
              await new SabyInfoRepository().UpdateStatus("KILLED", null, "ACTIVATION PROXMOX");
              return false;
          }
       ...
       //#endregion
        ...
        var appsReady =  await checker.isAppsReady();
         ...
         await this.delayFunc(1000);
           ...
           if(!whatsappStarted){
               await new SabyInfoRepository().UpdateStatus("KILLED",null,"CAN't OPEN WHATSAPP");
               return false;
           }
           ...
           if(await this.python.exists(`${IMG}profileINfo.png`,null,10) || await avdScript.SearchString("Profile info")) {
                  console.log("found Profile info")
                  await new SabyInfoRepository().Modify(true, id, GB, null,null, "Activated");
                  await new SabyInfoRepository().UpdateStatus("PROFILE INFO STUCK",null,'STUCK');
                  return  true;
           }

           await avdScript.dumpScreenToFile();
           const whatsappActivation = new WhatsappActivation(avdScript.getEmulatorPort());
          ...
          if(nameAndNumber){
           ...
           await new SabyInfoRepository().Modify(true, id, GB, nameAndNumber[0], nameAndNumber[1], null);
    ...
    async checkWhatsappImages(){
       ...
       return  !await this.python.exists(`${IMG}zoomWhatsapp.png`,null,5) || !await this.python.exists(`${IMG}WhatsAppImage.png`,null,5) || !await this.python.exists(`${IMG}whatsappZoomwhite.png`,null,5) || !await this.python.exists(`${IMG}WhatsappImageWhite.png`,null,5)
